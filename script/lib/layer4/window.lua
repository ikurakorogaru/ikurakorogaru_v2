...
local rect = require("script.lib.layer2.draw.rect").rect
...
function w.window(args)
    ...
    local mysprite = rotationPart:newSprite(args.name .. "_sprite")
    local function tick()
        ...
        datas.cursor = hit.hit(vec(datas.pos.x, datas.pos.y, datas.pos.z), right, up)
    end
    local function render(delta) if delta == nil then delta = 0 end end
